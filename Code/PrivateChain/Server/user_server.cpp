...
#ifndef USER_DEFAULT_STATIC_DIR
#define USER_DEFAULT_STATIC_DIR "../static"
#endif
...
bool read_request_line(int client_fd, std::string& method, std::string& target)
{
    std::string raw;
    char buffer[4096];
    while(raw.find("\r\n\r\n") == std::string::npos)
    ...
    return static_cast<bool>(line >> method >> target >> version);
}
...
bool write_all(int client_fd, const std::string& data)
{
    ...
    {
        const ssize_t count = write(client_fd, data.data() + written,
                                    data.size() - written);
...
void send_response(int client_fd, const std::string& status,
                   ...
                   bool send_body, const std::string& extra_headers = "")
{
                                            ...
                                            const std::string& target)
{
    ...
    const auto decoded = percent_decode(target.substr(0, query));
...
int main(int argc, char* argv[])
{
    ...
    while(true)
    {
        ...
        std::string method;
        std::string target;
        if(!read_request_line(client_fd, method, target))
        {
        ...
        {
            const auto file_path = resolve_static_path(static_root, target);
